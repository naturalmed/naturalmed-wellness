...
# ── Cross-language map (month -> slug_full) ───────────────────

def build_lang_map(content_dir):
    """
    Pre-scan an articles content dir and return a {'YYYY-MM': slug_full}
    map, used to link each article to its counterpart in the other
    language. Assumes at most one article per language per month
    (the site's current monthly publishing cadence).
    """
    import glob as _glob
    lang_map = {}
    for md_path in sorted(_glob.glob(str(content_dir / '*.md'))):
        ...
        lang_map[month_key] = slug_full
    return lang_map

def make_lang_toggle(lang, month_key, other_lang_map):
    """Build the EN/PT toggle markup, linking to the matching article
    in the other language when one exists for the same month."""
    other_slug = (other_lang_map or {}).get(month_key)
    if lang == 'pt':
        en_href = f'../../en/articles/{other_slug}.html' if other_slug else '#'
        return f'<div class="lang-toggle"><a href="{en_href}">EN</a><a href="#" class="active">PT</a></div>'
    pt_href = f'../../pt/artigos/{other_slug}.html' if other_slug else '#'
    return f'<div class="lang-toggle"><a href="#" class="active">EN</a><a href="{pt_href}">PT</a></div>'

# ── Core article builder (shared logic) ──────────────────────

def build_articles(content_dir, output_dir, template, lang='en', other_lang_map=None):
    """
    ...
    for md_path in md_files:
        ...
        lang_toggle = make_lang_toggle(lang, pub_date.strftime('%Y-%m'), other_lang_map)
...
def main():
    ...
    lang_map_en = build_lang_map(CONTENT_DIR_EN)
    lang_map_pt = build_lang_map(CONTENT_DIR_PT)
    articles_en = build_articles(CONTENT_DIR_EN, ARTICLES_DIR_EN, ARTICLE_TEMPLATE_EN, lang='en', other_lang_map=lang_map_pt)

    # ── 2. Build PT artigos ───────────────────────────────────
    print('\n── Building PT artigos ──')
    ARTICLES_DIR_PT.mkdir(parents=True, exist_ok=True)
    articles_pt = build_articles(CONTENT_DIR_PT, ARTICLES_DIR_PT, ARTICLE_TEMPLATE_PT, lang='pt', other_lang_map=lang_map_en)
